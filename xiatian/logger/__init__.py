...
class Logger:
    ...
    Args:
        ...
        when(str): 间隔生成文件的单位：'S':秒;'M':分;'H':小时;'D':天;'midnight':凌晨
    ...
    def __init__(self, filename, level='info', when='D', backCount=30):
        ...
        if not self.logger.handlers:
            # 不是每次生成一个新的logger，而是先检查内存中是否存在一个叫做filename的
            # logger对象，存在则取出，不存在则新建
...
if __name__ == "__main__":
    ...
    log_error = Logger("logs/test.log", level='error')
